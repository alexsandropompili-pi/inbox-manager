 ...
 *
 * Reads all messages where token_code IS NULL, calls Claude for each one
 * to extract notions and generate a token code, then updates the record.
 *
 * Usage:
...
async function tokenize(msg: Message): Promise<{
  ...
  return { token_code, notion_1: input.notion_1, notion_2: input.notion_2, notion_3: input.notion_3 }
}

// ─── Main ─────────────────────────────────────────────────────────────────────

async function main() {
  // Fetch all messages without a token
  ...
  if (!messages || messages.length === 0) {
    console.log('No messages without token_code found. Nothing to do.')
    return
  }
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    try {
      ...
      console.log(`${prefix} ✓ ${tokenData.token_code} — ${tokenData.notion_1}`)
      success++
    } catch (err) {
    ...
    // Small pause between calls to avoid rate-limit bursts
